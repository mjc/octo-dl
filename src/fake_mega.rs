...
mod tests {
    ...
    use super::{FakeMegaServer, benchmark_parallel_memory_download, create_fake_mega_fixture};
    use crate::{DownloadConfig, DownloadProgress, Downloader, NoProgress};
    ...
    #[tokio::test]
    async fn benchmark_memory_path_verifies_condensed_mac() {
        let temp = tempdir().unwrap();
        let fixture_dir = temp.path().join("fixture");
        let fixture = create_fake_mega_fixture(&fixture_dir, "bench.bin", 1_572_864, 23)
            .await
            .unwrap();
        let server = FakeMegaServer::spawn(fixture.clone(), 2).unwrap();
        ...
        benchmark_parallel_memory_download(&http, &download_base_url, node, 2, 4)
            .await
            .unwrap();
